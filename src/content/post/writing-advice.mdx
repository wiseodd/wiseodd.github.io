...
<Admonition kind='error'>
  **DO NOT** use LLMs when writing. It should only be used to proofread, i.e., **after**
  you're done. Using an LLM to write or draft is akin to going to the gym and letting
  your personal trainer lift the weights for you. (True for general
  learning/exercises/practices.)
</Admonition>
...
<Admonition kind='info'>
  You can use the following editors: (i) Local LaTeX with git for collab: Check LaTeX
  workshop if you use VSCode, or VimTex if you use Neovim. (ii)
  [Crixet](https://crixet.com/). (ii) Overleaf.
</Admonition>
