import Admonition from "@/components/Admonition.astro";

<Admonition kind='warning'>
    Currently, I do **not** have any opening for this.
</Admonition>

1. See **Direct-Entry PhD** requirements below. They must be fulfilled. Additional requirements below.
2. You have at least a first-author conference (not workshop!) submission (doesn't have to be accepted yet!) in ML venues like AISTATS, UAI, NeurIPS, ICML, and ICLR.
3. You have at least one reference letter from a researcher/prof in ML who publishes regularly in the above venues.
4. You have a very clear, vivid motivation why you want to obtain a PhD.
5. You are highly-disciplined and reliable.
